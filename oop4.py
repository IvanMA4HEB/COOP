...
class Manager:
    def prikol_cyka(self):
        ...
        if (storage.storageNames.index(order.orderProducts)) == (storage.storageQuan.index(storage.quantity)):
            print(f'{storage.storageNames.index(order.orderProducts)} = {storage.storageQuan.index(storage.quantity)}')
